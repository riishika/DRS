...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    log(`Request received — file: ${media.name} (${(media.size / 1024 / 1024).toFixed(2)} MB, type: ${media.type})`);
    ...
    if (isImage(media)) {
      ...
      analysis = await analyzeUploadedImage(media);
      ...
      analysis = await analyzeUploadedVideo(media);
